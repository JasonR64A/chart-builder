...
# Top 10 Portal Entrants — Weekly-Awards template ranked by the 64 Analytics
# PORTAL RANK (portal_rank_player.sixty_four_rating_portal_player). Pool = 2026
# portal entrants whose entry date (player_rank.initiated_date) is in a chosen
# date range. Simple: pick a date range, get the top 10 by portal rank.
if view == 'Top 10 Portal Entrants':
    ...
    st.caption('Top 10 players to enter the 2026 portal within a date range, ranked by their '
               '64 Analytics portal rank.')

    today = pd.Timestamp.now().normalize()
    # ── Entrant date range ──
    pe_range = st.sidebar.date_input(
        'Entrant date range',
        value=((today - pd.Timedelta(days=6)).date(), today.date()), key='pe_range',
        help='Portal entrants whose entry date (initiated_date) is in this window.')
    if isinstance(pe_range, (tuple, list)) and len(pe_range) == 2:
        pe_start, pe_end = pd.Timestamp(pe_range[0]), pd.Timestamp(pe_range[1])
    else:
        st.info('Pick a start and end date.')
        st.stop()

    # Entrants in the window (player_rank.initiated_date) -> cb player_ids
    _pr = pd.read_csv(DATA_DIR / 'player_rank.csv', encoding='utf-8-sig', low_memory=False,
                      usecols=lambda c: c in ('player_id', 'year', 'in_portal', 'initiated_date'))
    _pr = _pr[(pd.to_numeric(_pr['year'], errors='coerce') == 2026)
              & (_pr['in_portal'].astype(str).str.upper() == 'TRUE')].copy()
    _pr['_dt'] = pd.to_datetime(_pr['initiated_date'], errors='coerce')
    _pr = _pr[(_pr['_dt'] >= pe_start) & (_pr['_dt'] < pe_end + pd.Timedelta(days=1))]
    in_window = set(pd.to_numeric(_pr['player_id'], errors='coerce').dropna().astype(int))

    # Portal rank + name + team from portal_rank_player.csv (2026)
    prp = pd.read_csv(DATA_DIR / 'portal_rank_player.csv', encoding='latin-1', low_memory=False)
    prp = prp[pd.to_numeric(prp['year'], errors='coerce') == 2026].copy()
    prp['player_id'] = pd.to_numeric(prp['player_id'], errors='coerce').astype('Int64')
    prp['PortalRank'] = pd.to_numeric(prp['sixty_four_rating_portal_player'], errors='coerce')
    prp = prp[prp['player_id'].isin(in_window) & prp['PortalRank'].notna() & (prp['PortalRank'] > 0)]

    # Restrict to the page's sport (entrant team -> teams.csv sport) + attach school
    teams_df = pd.read_csv(DATA_DIR / 'teams.csv', low_memory=False)
    _tid = pd.to_numeric(teams_df['id'], errors='coerce')
    team_sport = dict(zip(_tid, teams_df['sport'].astype(str).str.lower()))
    ...
    if pe_mode.startswith('Manual'):
        ...
        if not _picked:
            ...
            st.stop()
        df_render = df_sorted.iloc[[_lab2idx[l] for l in _picked]].reset_index(drop=True)
    else:
        df_render = df_sorted.head(10).copy()
